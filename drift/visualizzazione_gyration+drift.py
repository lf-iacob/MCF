...
def em_exb(E, B, q, m, v0_x, v0_y, v0_z, N):
    ...
    field=E/B
    omega_c=(abs(q)*B)/m
    v_ort_g=np.sqrt(v0_x**2+v0_y**2)
    theta_g=np.arccos(v0_x/v_ort_g)
    ...
    #inizio simulazione: aggiornamento della velocità
    for j in range(0, N):
        vxi_g=v_ort_g*np.cos(omega_c*j*t+theta_g)
        vyi_g=-(q/abs(q))*v_ort_g*np.sin(omega_c*j*t+theta_g)
        vx_g=np.append(vx_g, vxi_g)
        vy_g=np.append(vy_g, vyi_g)
        xt_g=xt_g+vxi_g*t
        yt_g=yt_g+vyi_g*t
        x_g=np.append(x_g, xt_g)
    ...
    plt.plot(vx_g, vy_g, marker='.', color='orange', label='Gyration')
    plt.legend()
    plt.title('v_ortogonale (t)')
    plt.xlabel('vx (m/s)')
    plt.ylabel('vy (m/s)')
    ...
    fig.suptitle('Leggi di velocità su ogni dimensione', fontsize=25, y=1)
    ax[0].set_title('VX', fontsize=15)
    ax[0].plot(time, vx, color='mediumblue', label='Drift ExB')
    ax[0].plot(time, vx_g, color='orange', label='Gyration')
    ax[0].legend()
    ax[0].grid(linestyle=':')
    ax[0].set_xlabel('Tempo (s)', fontsize=10)
    ax[0].set_ylabel('vx(t) (m)', fontsize=10)
    ...
    ax.scatter(vx_g, vy_g, vz, marker='.', c=vz, cmap='viridis', label='Gyration')
    ax.legend()
    ax.set_aspect('equal')
    ...
    plt.plot(x_g, y_g, marker='.', color='orange', label='Gyration')
    plt.legend()
    plt.title('Traiettoria ortogonale (t)')
    plt.xlabel('x (m/s)')
    plt.ylabel('y (m/s)')
    ...
    ax[0].set_title('X', fontsize=15)
    ax[0].plot(time, x, color='mediumseagreen', label='Drift ExB')
    ax[0].plot(time, x_g, color='orange', label='Gyration')
    ax[0].legend()
    ax[0].grid(linestyle=':')
    ax[0].set_xlabel('Tempo (s)', fontsize=10)
    ax[0].set_ylabel('x(t) (m)', fontsize=10)
    ...
    ax.scatter(x_g, y_g, z, c=time, cmap='viridis', label='Gyration')
    ax.plot(x_g, y_g, z, color='orange', lw=0.7, label='Gyration')
    ...
    print('Gyration: ({:}, {:}, {:})'.format(vx_g[0], vy_g[0], vz[0]))
